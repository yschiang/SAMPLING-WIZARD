...
def test_grid_uniform_common_edge_exclusion():
    """
    Test edge_exclusion_mm from common config (v1.3).

    Verifies that additional edge exclusion filters grid points near edge.
    """
    strategy = GridUniformStrategy()
    ...
    # Grid should maintain uniform distribution within available region
    # Verify at least some points selected
    assert len(points) >= 5

    print(f"✅ COMMON EDGE_EXCLUSION: All {len(points)} points within 120mm (30mm exclusion), uniform distribution")


def test_grid_uniform_common_rotation_seed():
    """
    Test rotation_seed from common config (v1.3).

    Verifies that rotation affects grid angular ordering.
    """
    strategy = GridUniformStrategy()
    ...
    # Both should maintain center-to-edge ordering (sorted by distance ascending)
    for result in [result_no_rotation, result_rotated]:
        points = result.selected_points
        distances = [math.sqrt((p.die_x * 10.0)**2 + (p.die_y * 10.0)**2) for p in points]
        # Check monotonically non-decreasing (center to edge)
        for i in range(len(distances) - 1):
            assert distances[i] <= distances[i+1] + 0.01

    print(f"✅ COMMON ROTATION: No rotation={len(result_no_rotation.selected_points)} points, 90° rotation={len(result_rotated.selected_points)} points (deterministic, center-to-edge preserved)")


def test_grid_uniform_common_target_point_count():
    """
    ...
    # Should maintain center-to-edge ordering
    distances = [math.sqrt((p.die_x * 10.0)**2 + (p.die_y * 10.0)**2) for p in points]
    for i in range(len(distances) - 1):
        assert distances[i] <= distances[i+1] + 0.01
    ...
    print(f"✅ COMMON TARGET_POINT_COUNT: Requested 12, got {len(points)} (uniform stride)")


def test_grid_uniform_common_config_integration():
    """
    ...
    Note: deterministic_seed for jitter is not yet implemented in GRID_UNIFORM.
    Jitter tests deferred until jitter feature is added.
    """
    strategy = GridUniformStrategy()
    ...
    # Verify center-to-edge ordering
    distances = [math.sqrt((p.die_x * 10.0)**2 + (p.die_y * 10.0)**2) for p in points]
    for i in range(len(distances) - 1):
        assert distances[i] <= distances[i+1] + 0.01
    ...
    print(f"✅ COMMON CONFIG INTEGRATION: {len(points)} points with edge_exclusion=20mm, rotation=45°, target=15 (uniform)")
...
if __name__ == "__main__":
    ...
    test_grid_uniform_common_edge_exclusion()
    test_grid_uniform_common_rotation_seed()
    test_grid_uniform_common_target_point_count()
    test_grid_uniform_common_config_integration()
